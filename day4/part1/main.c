...
int main(int argc, char **argv)
{
    ...
    if (row_or_col == ROW) {
        printf("Board: %d, Row: %d\n", right_board_index, right_col_or_row_index);
        for(i = 0; i < ROW_SIZE; i++) {
            int curr = boards[right_board_index][(ROW_SIZE * right_col_or_row_index) + i];
        ...
        printf("Board: %d, Col: %d\n", right_board_index, right_col_or_row_index);
        for(i = 0; i < ROW_SIZE; i++) {
            int curr = boards[right_board_index][right_col_or_row_index + (i * ROW_SIZE)];
            // If the number is bigger it came later 
            if (cached_bingo_nums[curr] > last_bingo_num_index) {
                last_bingo_num_index = cached_bingo_nums[curr];
                last_bingo_num = curr;
    ...
    for(i = 0; i < ROW_SIZE * ROW_SIZE; i++) {
        int curr = boards[right_board_index][i];
